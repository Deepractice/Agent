...
export function messageStop(agentId: string, stopReason?: string, stopSequence?: string): MessageStopEvent {
  return {
    ...createEventBase(agentId),
    type: "message_stop",
    data: {
...
export function toolResult(agentId: string, toolId: string, content: string | any[], isError: boolean): ToolResultEvent {
  return {
    ...createEventBase(agentId),
    type: "tool_result",
