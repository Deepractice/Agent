...
└── packages/
    ├── agentx-core/      # Session management (AgentRegistry, SessionStore)
    ├── agentx-engine/    # Runtime engine (AgentService, AgentEngine, EventBus)
    ├── agentx-framework/ # Framework API (defineAgent, defineDriver, SSEServer)
...
│    ↓ depends on                                                  │
├─────────────────────────────────────────────────────────────────┤
│  agentx-core (AgentRegistry, SessionStore)                      │
│    ↓ depends on                                                  │
├─────────────────────────────────────────────────────────────────┤
│  agentx-engine (AgentService, AgentEngine, EventBus)            │
│    ↓ depends on                                                  │
├─────────────────────────────────────────────────────────────────┤
│  agentx-event, agentx-types, agentx-logger (基础类型和工具)       │
└─────────────────────────────────────────────────────────────────┘
```
...
- `AgentRegistry` (`packages/agentx-core/src/AgentRegistry.ts`) - Agent 生命周期管理
...
SSEServer 使用双 Manager 架构管理生命周期：

```
┌─────────────────────────────────────────────────────────────────┐
│  SSEServer                                                      │
│  ┌─────────────────────────┐  ┌─────────────────────────────┐  │
│  │ AgentRegistry           │  │ SSEConnectionManager        │  │
│  │ (from agentx-core)      │  │ (agentx-framework)          │  │
│  │                         │  │                             │  │
│  │ sessionId →             │  │ sessionId →                 │  │
│  │   AgentService          │  │   SSEConnection             │  │
│  │   - queue()             │  │   - reactor                 │  │
│  │   - registerReactor()   │  │   - pendingMessages         │  │
│  └─────────────────────────┘  └─────────────────────────────┘  │
│           │                            │                        │
│           │ onSessionDestroy ──────────→ destroy()              │
│           │ (Agent销毁时清理SSE)                                 │
└─────────────────────────────────────────────────────────────────┘
```

**生命周期关系**: 1 Session : 1 Agent : N SSE Connections (支持断线重连)
...
3. POST /api/message      → 发送消息，通过 SSE 接收响应
```
